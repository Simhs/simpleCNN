...
class CNN:
    ...
    def whatisthis(self,cv2_img,type = "BGR"):
        ...
        result = self.sess.run(tf.nn.softmax(self.pred), feed_dict={self.x: pic, self.keep_prob: 1.})
        result_list = (list(result)[0])
        mlist = list(result_list)
        print mlist ,int(mlist.index(max(mlist)))
...
if __name__ =="__main__":
    ...
    for i in range(50):
        cnn.addTraningSet("./0/new_data/"+str(i)+".jpg", 0) #param("./john/picname.jpg",(0~n_classes))

    for i in range(50):
        cnn.addTraningSet("./1/new_data/"+str(i)+".jpg", 1) #param("./john/picname.jpg",(0~n_classes))
    ...
    cnn.save()
    test_img = cv2.imread("./0/new_data/51.jpg")
    cnn.whatisthis(test_img)
    test_img = cv2.imread("./1/new_data/51.jpg")
    cnn.whatisthis(test_img)
        ...
        if len(faces) != 0:
            for (x, y, w, h) in faces:
                ...
                if  temp == 0:
                    ...
                    cv2.rectangle(smallImg, (x, y), (x + w, y + h), (0, 255, 0), 2)
